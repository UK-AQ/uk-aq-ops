...
import { assertHistoryPayload, historyResolution, resolveHistoryEnvironment } from "./history_generation";
...
async function rows(env: WorkerEnv, base: string, table: string, h: Headers, params: Record<string,string>): Promise<JsonObject[]> { const target = url(base, table, params); const startedAt = Date.now(); const r = await fetch(target, { headers: h }); const t = await r.text(); recordDashboardSupabaseResponse(env, target, r, t, Date.now() - startedAt); if (!r.ok) throw new Error(`${table} (${r.status}): ${t}`); const j = t ? JSON.parse(t) : []; return Array.isArray(j) ? j.filter((x): x is JsonObject => x && typeof x === "object" && !Array.isArray(x)) : []; }
async function apiRows(apiUrl: string, token: string, params: Record<string,string>, candidates: string[], expectedReadVersion: "v2" | "v3", resolvedEnv?: WorkerEnv): Promise<JsonObject[]> {
  if (!apiUrl) return [];
  const u = new URL(apiUrl);
  for (const [k,v] of Object.entries(params)) u.searchParams.set(k,v);
  const h = new Headers(); if (token) h.set("x-uk-aq-upstream-auth", token);
  const r = await fetch(u.toString(), { headers: h }); const t = await r.text(); if (!r.ok) throw new Error(`${apiUrl} (${r.status}): ${t}`);
  const j = t ? JSON.parse(t) : [];
  if (!j || typeof j !== "object" || Array.isArray(j)) throw new Error("History API returned an invalid payload");
  if (resolvedEnv) assertHistoryPayload(j as JsonObject, resolvedEnv);
  else if (j.read_version !== expectedReadVersion || j.error || j.ok === false) throw new Error("Legacy history payload does not match requested generation or reports failure");
...
export async function handleStationSnapshotV2Search(env: WorkerEnv, search: URLSearchParams): Promise<Response> {
  ...
  const result = await rows(env, base, "stations", h, { select: "id,station_ref,label,name,connector_id,connectors(id,label,name)", or: `(label.ilike.*${safe}*,name.ilike.*${safe}*,station_ref.ilike.*${safe}*,id.eq.${/^\d+$/.test(safe) ? safe : -1})`, order: "label.asc", limit: "20" });
...
export async function handleStationSnapshotV2Rows(env: WorkerEnv, search: URLSearchParams): Promise<Response> {
  ...
  const ts = await rows(env, base, "timeseries", h, { select: "id,connector_id,station_id,label,uom,observed_property_id,observed_properties(code)", station_id: `eq.${stationId}`, order: "id.asc", limit: "100" });
  ...
  const ingestObservs = await rows(env, base, "observations", h, { select: "timeseries_id,observed_at,value", timeseries_id: `eq.${selected}`, observed_at: `gte.${start}`, order: "observed_at.desc", limit: "3000" });
  ...
  const r2BaseParams = { timeseries_id: String(selected), connector_id: selectedConnectorId, pollutant, since: start, since_utc: start, from_utc: start, start_utc: start, to_utc: end, end_utc: end, debug: "1" };
  let r2ObservationParams: Record<string, string> = { ...r2BaseParams, read_version: "unresolved" };
  try { const obsApi = String(env.UK_AQ_OBSERVS_HISTORY_R2_API_URL || "").trim(); const resolvedEnv = await resolveHistoryEnvironment(env); const readVersion = historyResolution(resolvedEnv).version; r2ObservationParams = { ...r2BaseParams, read_version: readVersion }; r2Observs = await apiRows(obsApi, String(env.UK_AQ_OBSERVS_HISTORY_R2_API_TOKEN || "").trim(), r2ObservationParams, ["rows", "observations"], readVersion, resolvedEnv); } catch (_err) {}
  const r2AqiParams = { ...r2BaseParams, read_version: "v2", format: "objects" };
  try { const aqiApi = String(env.UK_AQ_AQI_HISTORY_R2_API_URL || "").trim(); r2Aqi = await apiRows(aqiApi, String(env.UK_AQ_AQI_HISTORY_R2_API_TOKEN || "").trim(), r2AqiParams, ["rows", "data", "points"], "v2").then(a => a.map(r => ({...r, observed_at: r.observed_at || r.timestamp_hour_utc || r.period_start_utc, hourly_mean_ugm3: r.hourly_mean_ugm3 ?? r.eaqi_input_value_ugm3 ?? r.daqi_input_value_ugm3, rolling24h_mean_ugm3: r.rolling24h_mean_ugm3 ?? r.daqi_input_value_ugm3, hourly_sample_count: r.hourly_sample_count ?? r.source_observation_count }))); } catch (_err) {}
  try { const ob = restBase(env, true); const oh = headers(serviceKey(env, true), String(env.UK_AQ_PUBLIC_SCHEMA || "uk_aq_public")); obsAqidbObservs = await rows(env, ob, "observations", oh, { select: "timeseries_id,observed_at,value", timeseries_id: `eq.${selected}`, observed_at: `gte.${start}`, order: "observed_at.desc", limit: "3000" }); } catch (_err) {}
  try { const ob = restBase(env, true); const oh = headers(serviceKey(env, true), String(env.UK_AQ_PUBLIC_SCHEMA || "uk_aq_public")); ingestAqi = await rows(env, ob, "uk_aq_timeseries_aqi_hourly", oh, { select: "timeseries_id,timestamp_hour_utc,hourly_mean_ugm3,rolling24h_mean_ugm3,hourly_sample_count,daqi_index_level,eaqi_index_level", timeseries_id: `eq.${selected}`, timestamp_hour_utc: `gte.${start}`, order: "timestamp_hour_utc.desc", limit: "3000" }).then(a => a.map(r => ({...r, observed_at: r.timestamp_hour_utc}))); } catch (_err) {}
