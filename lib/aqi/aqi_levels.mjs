...
export function expandPmHelperRowsToHourlyGrid(helperRows) {
  ...
  for (const rows of byTimeseries.values()) {
    ...
    // Synthetic PM hours are valid only inside an observed series.  Never
    // extend before its first or after its latest actual observation.
    const startMs = Date.parse(rows[0].timestamp_hour_utc);
    const endExclusiveMs = Date.parse(rows[rows.length - 1].timestamp_hour_utc) + HOUR_MS;
